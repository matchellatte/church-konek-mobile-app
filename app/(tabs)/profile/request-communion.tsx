...
import { SafeAreaView, Text, TextInput, TouchableOpacity, StyleSheet, Alert, ActivityIndicator, ScrollView } from 'react-native';
import { Calendar } from 'react-native-calendars'; // Calendar picker
import { supabase } from '../../../backend/lib/supabase';
import { useRouter } from 'expo-router';

const RequestCommunion = () => {
  const router = useRouter();
  const [name, setName] = useState(''); // Name for first communion
  const [selectedDate, setSelectedDate] = useState(''); // Date of first communion
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('certificate_requests').insert({
        email,
        certificate_type: 'First Communion',
        date_of_first_communion: selectedDate,
        details: `Name: ${name}, Additional Info: ${details}`,
        status: 'Pending',
      });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>First Communion Certificate Request</Text>

        <Text style={styles.label}>Name of the Communicant:</Text>
        <TextInput
          style={styles.input}
        ...
        <Text style={styles.label}>Date of First Communion:</Text>
        <Calendar
...
export default RequestCommunion;
